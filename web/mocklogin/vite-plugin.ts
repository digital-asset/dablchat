...
function renderPublicToken(res: http.ServerResponse) {
  daml.listKnownParties().then((parties) => {
    ...
    const access_token = mintToken({
      sub: "anonymous",
      party: publicPartyDetails!.identifier,
    });
...
function renderDefaultParties(res: http.ServerResponse) {
  daml.listKnownParties().then((parties) => {
    const result = parties.filter(
      (p) => p.displayName === "Public" || p.displayName === "UserAdmin",
    );
    res.setHeader("Content-Type", "application/json");
    res.setHeader(
      "Cache-Control",
      ...
      server.middlewares.use((req, res, next) => {
        ...
        switch (pathname) {
          case "/.hub/v1/auth/login":
            renderHtmlPage(
              res,
              fs.readFileSync(`${__dirname}/v1.html`, "utf-8"),
            );
            break;
          ...
          case "/.hub/v2/auth/login":
            renderHtmlPage(
              res,
              fs.readFileSync(`${__dirname}/v2.html`, "utf-8"),
            );
            break;
