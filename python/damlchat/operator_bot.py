...
def main():
    ...
    def post_message_command(user, message):
        return exercise_by_key(
            Chat.Chat,
            {"_1": client.party, "_2": user},
            "Chat_PostMessage",
            {"poster": client.party, "message": message, "postedAt": int(time.time())},
        )
    ...
    def create_operator(_):
        ...
        if not res:
            ...
            return client.submit_create(
                Chat.Operator, {"operator": client.party, "publicParty": public_party}
            )
        else:
            logging.info(f"Operator {party} is ready")
            user_account_requests = client.find_active(Chat.UserAccountRequest)
            logging.info(
                f"found {len(user_account_requests)} {Chat.UserAccountRequest} contracts"
            )
            ...
            logging.info(
                f"found {len(request_cids_to_accept)} requests to accept "
                f"and {len(request_cids_to_archive)} duplicated requests to archive"
            )

            commands_to_run = [
                exercise(cid, "UserAccountRequest_Accept")
                for cid in request_cids_to_accept
            ] + [
                exercise(cid, "UserAccountRequest_Reject")
                for cid in request_cids_to_archive
            ]
    ...
    def invite_users(_):
        ...
        return [
            exercise(cid, "UserAccountRequest_Accept")
            for cid in user_account_requests.keys()
        ]
    ...
    def send_to_slack_channel(event):
        ...
        posted_at = time.strftime(
            "%a, %d %b %Y %H:%M:%S %Z", time.localtime(float(cdata["postedAt"]))
        )
        message_text = (
            f"`From:` {cdata['sender']}\n`Posted At:` {posted_at}\n"
        ...
        return [
            create(
                SlackIntegration.OutboundMessage,
                {
    ...
    def divulge_aliases(event):
        ...
        mappings = [
            f"{cdata['user']} -> {cdata['alias']}" for _, cdata in aliases.items()
        ]
        mappings_str = "\n".join(mappings)
        if len(mappings) > 0:
            known_users_message = (
                f"Here is the list of known users:\n```\n{mappings_str}\n```"
            )
        else:
            ...
            if new_user not in cdata["members"]:
                commands.append(
                    exercise(
                        cid,
                        "Chat_AddMembers",
                        {"member": client.party, "newMembers": [new_user]},
                    )
                )
    ...
    def respond_to_chat_request(event):
        ...
        if party_members:
            return client.submit_exercise(
                event.cid,
                "CreateChatRequest_Respond",
                {"partyMembers": party_members, "chatId": str(uuid.uuid4())},
            )
        else:
