...
import dazl
from dazl import ContractId, exercise
...
def main():
    ...
    archive_state = ArchiveState(
        message_heap=[], archive_after=int(timedelta(days=14).total_seconds())
    )
    ...
    def expired(after: int, posted_at: int) -> bool:
        return (
            datetime.fromtimestamp(posted_at) + timedelta(seconds=after)
        ) < datetime.now()
    ...
    async def bot_ready(_):
        ...
        archive_state.message_heap = [
            Message(cdata["postedAt"], cid) for cid, cdata in existing_messages.items()
        ]
        ...
        (_, settings_cdata) = await client.find_one(
            Chat.UserSettings, {"user": client.party}
        )
    ...
    async def archive_stale_messages(event):
        ...
        try:
            ...
            commands = [
                ...
                if expired(settings_cdata["archiveMessagesAfter"], cdata["postedAt"])
            ]
    ...
    async def message_heapify(event):
        ...
        if event.cdata["sender"] == client.party:
            ...
            heapq.heappush(message_heap, Message(event.cdata["postedAt"], event.cid))
